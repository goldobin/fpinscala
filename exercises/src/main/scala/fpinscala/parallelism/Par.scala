...
import fpinscala.parallelism.Par.sequence
...
object Par {
  ...
  def sequence[A](ps: List[Par[A]]): Par[List[A]] = {
    ps.foldRight(Par.unit(List[A]())) { (par, result) =>
      map2(par, result) { (a, list) =>
        a :: list
      }
    }
  }
  ...
  def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] = fork {
    ...
    val result = sequence(as.map(asyncF(mapF)))

    map(result) { as =>
      as.filter(v => v._1).map(v => v._2)
    }
  }
